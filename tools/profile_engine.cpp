...
int main() {
    for (std::size_t count : {100U, 200U, 300U}) {
        ...
        for (std::size_t i = 0; i < count; ++i) room.furniture.push_back({"f" + std::to_string(i), "box", {{static_cast<float>(i % 40) + .5F, 0, static_cast<float>(i / 40) + .5F}, {}, {1, 1, 1}}, {.8F, 1.0F, .8F}, "mat"});
